        ...
        @if ($loop->index >= 3)
            <button
                ...
                <a
                   ...
                   href="/?tags={{ $tag->slug }}"
                   class="px-2 py-1 text-xs font-semibold uppercase break-normal border-2 border-solid rounded-full"
                   style="font-size: 8px; border-color: {{ $tag->category->style }}; color: {{ $tag->category->style }};">{{ $tag->name }}</a>
            </button>
        @endif
    @endforeach
    <button
            class="px-3 py-1 transition duration-150 ease-in bg-gray-100 border rounded-full hover:bg-gray-200 h-7"
            @click="isOpen = !isOpen">
